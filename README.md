...
`Pool.map()` is a great tool for getting true parallel performance as each process is its own interpreter and [GIL](https://docs.python.org/3/glossary.html#term-global-interpreter-lock).
This is especially important when the list of items is very large. The Dragon version of this code looks like the following:
...
Since we can manage the life-cycle of `mp.Pool()` explicitly, we can have them close and bring up new ones as our computational load changes. Dragon users 
can view their nodes as a single collection of resources and program different elements of their application to use varying amounts of
resources over time, enabling a cloud-like experience.

In addition to enhancing `mp.Pool()` to supercomputer scales, Dragon also lets users do something base `multiprocessing` doesn't let you do: nest `mp.Pool()`
inside of one another. Pools that use Pool?! Why might you want that? There are many use-cases. Imagine you need to process different types of
data as they land in a filesystem and that each file has many components that require `Pool.map()`-like operations. Something like this:
...
There is much more you can do with `mp.Pool`, especially when an entire supercomputer's resources are at your command. The key thing is that with Dragon's
...
We often use it when multiple readers and/or writers need to communicate. It's a FIFO-style queue, and with Dragon's implementation, processes
...
scales well. We have designs for even better scaling, but that's for a different document. Here's how to use `mp.Queue` in combination with another idiom from
...
all communication and collective primitives in `multiprocessing`. Dragon's implementation relies on our high-performance (Shared memory+RDMA-capable) communication layer,
called [`Channels`](https://dragonhpc.github.io/dragon/doc/_build/html/ref/core/index.html#channels).

## Data
The Python `dict` is one of the most fundamental and useful abstractions in the language, in our opinon. What if we had a `dict` that scaled to hundreds or thousands of nodes
...
You can think of `DDict` as a co-located object store that scales with your application. For example, you might read in a large quanitity of
data from a filesystem and store them into the `DDict` with keys mimicking file paths. If you don't have a great parallel filesystem, this lets you read the data once,
cache it in the memory of your nodes, and leverage your network's performance (and shared memory) for subsequent accesses. Another use case: write intermediate results to a `DDict`, rather than using
the filesystem. If your workload consists of stages of Python processes in a pipeline, `DDict` is a very convenient way to manage data exchange
...
key/value pairs into the `DDict`. The keys are always 128 bytes in size, but the values vary in length. The plot below shows the aggregate
...
like SHMEM may be faster, but for the largest value sizes `DDict` is approaching 30% of the achievable bandwidth. We have seen high-end filesystems do worse. Note:
the `DDict` is not yet persistent between executions of Dragon. We are working on that feature.
